...
def run_policy(args):
    ...
    for rollout_id in range(num_rollouts):
        ...
        csv_writer.writerow(['timestep', 'base_vel_x', 'base_vel_y', 'actual_dt', 'scaled_vel_x', 'scaled_vel_y', 'time_scale'])
        ...
        with torch.inference_mode():
            ...
            prev_time = time0
            actual_dt_history = []
            prev_duration = DT  # Track previous step's processing time for velocity scaling
            ...
            for t in range(max_timesteps):
                ...
                print(f"Step {t}: base_action = {base_action}, prev_duration = {prev_duration:.3f}s")
                actual_dt_history.append(actual_dt)
                
                # CRITICAL FIX: Scale base velocity by time ratio to compensate for slow inference
                # Use PREVIOUS step's processing time to scale CURRENT velocity
                # Policy expects DT=0.02s per step, but actual steps take ~0.085s
                # Without scaling: 0.6 m/s * 0.085s = 0.051m traveled (should be 0.012m!)
                time_scale = DT / max(prev_duration, DT)  # Never scale up, only down
                base_action_scaled = base_action * time_scale
                
                if time_scale < 0.95:  # Log significant scaling
                    print(f"  -> Scaled velocity by {time_scale:.3f}x: {base_action} -> {base_action_scaled}")
                ...
                csv_writer.writerow([
                    ...
                    float(base_action[1]),
                    actual_dt,
                    float(base_action_scaled[0]),
                    float(base_action_scaled[1]),
                    time_scale
                ])
                
                # Step environment with SCALED velocity
                if IS_MOBILE:
                    ts = env.step(target_qpos, base_action_scaled, get_base_vel=True)
                else:
                    ts = env.step(target_qpos, None, get_base_vel=False)
                
                # Timing control
                duration = time.time() - time1
                prev_duration = duration  # Store for next iteration's velocity scaling
